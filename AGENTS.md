...
| `swift/docs/SWIFT_STORAGE.md` | app JSON stores, the shared modality-separated engine model store, `ModelStorageManager`, `AppStorageRoot` test isolation, and which directories tests may write | adding a store, touching a `~/.turbospark` path, changing model discovery, or testing catalog-backed storage |
