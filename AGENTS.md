...
| `swift/docs/SWIFT_CONTEXT_REFERENCES.md` | the typed `@` context references (`@diff`, `@staged`, `@git:N`, `@url:`, `@file:` line ranges, `@folder:`): grammar, resolution, the sensitive-path and SSRF gates, and the reserved-word escape | touching `ContextReferenceParser`, `MentionResolver`'s scheme arms, or the composer's reference rows |
...
| `swift/docs/storage.md` | app JSON stores, the configurable modality-separated engine model store, move wizard, provider discovery, `ModelStorageManager`, `AppStorageRoot` test isolation, and which directories tests may write | adding a store, touching a `~/.turbospark` path, changing model discovery, or testing catalog-backed storage |
...
| `swift/docs/SWIFT_PASTE_HANDLING.md` | large pastes: the 4,000-char chip conversion, suppression of programmatic draft writes, the head+tail transcript gate, over-window truncation | touching `PromptPastePolicy`, `AppModel+Paste`, `writePromptTextDirectly`, `MessageContentPreview`, `CollapsibleMessageContentView`'s gate, or a paste threshold |
