...
- `chat_template/`: Built-in per-dialect text chat template rendering (the fallback path).
- `jinja_chat_template.rs`: Jinja template engine wrapper (`minijinja` + `pycompat`) for rendering the checkpoint's own `chat_template.jinja`.
...
- `structured_decoder/`: `StructuredAssistantDecoder` for streaming JSON and structured output parsing.
