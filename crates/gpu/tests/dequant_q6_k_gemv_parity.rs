...
//! reference in `mrefrust_compute::dequant_q6_k_gemv` (ROADMAP Phase G
...
fn matches_cpu_reference() {
    ...
    let rows: Vec<Vec<u8>> = (0..m)
        .map(|r| mrefrust_compute::quantize_q6_k(&weights(n, 7 + r as u32)))
        .collect();
    ...
    let cpu = mrefrust_compute::dequant_q6_k_gemv(&refs, &x_f32, n);
    ...
    let rows: Vec<Vec<u8>> = (0..m)
        .map(|r| {
            ...
            let w: Vec<f32> = (0..n)
                ...
                .collect();
            mrefrust_compute::quantize_q6_k(&w)
        })
        .collect();
    let refs: Vec<&[u8]> = rows.iter().map(|r| r.as_slice()).collect();
...
fn the_bias_of_thirty_two_is_what_carries_the_sign() {
    ...
    let row = mrefrust_compute::quantize_q6_k(&w);
    ...
    let rows: Vec<Vec<u8>> = (0..m)
        .map(|r| {
            let scaled: Vec<f32> = weights(n, 41 + r as u32)
                ...
                .collect();
            mrefrust_compute::quantize_q6_k(&scaled)
        })
        .collect();
    let refs: Vec<&[u8]> = rows.iter().map(|r| r.as_slice()).collect();
...
fn the_resident_form_matches_the_copying_form() {
    ...
    let m = 4usize;
    let rows: Vec<Vec<u8>> = (0..m)
        .map(|r| mrefrust_compute::quantize_q6_k(&weights(n, 13 + r as u32)))
        .collect();
    let refs: Vec<&[u8]> = rows.iter().map(|r| r.as_slice()).collect();
