...
// and `ple_gate_fp16`'s shape), repeated for each query head so ReLU is
// applied to each head's dot product before those scores are summed.
[[kernel, max_total_threads_per_threadgroup(256)]]
void qsa_score_blocks_fp16(
    ...
    for (uint h = 0; h < num_heads; h++) {
        ...
        for (uint dim = lid; dim < d; dim += lsize) {
            acc = fma(float(q[h * d + dim]), float(pooled_block[dim]), acc);
        }
        acc = simd_sum(acc);
        if (simd_lane_id == 0) {
