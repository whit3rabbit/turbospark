...
//
// THIS FILE IS COMPILED CONCATENATED AFTER `moe.metal`, which is where
// `RoutedBlobs`, `ExpertOffsets`, `moe_hidden_activation` and the
// `moe_fc_*` function-constant accessors come from. It never appears on its
// own: `crates/gpu/src/moe_gguf.rs` passes `concat!` of the two as ONE
