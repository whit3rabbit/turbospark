...
|   +-- kv_cache_rewind.rs          # KvCacheManager's rewind and reset methods (split from kv_cache.rs)
|   +-- kv_quant_tables.rs          # TurboQuant sign/codebook/midpoint tables, built once at open
|   +-- kv_quantize.rs              # TurboQuant KV commit quantization dispatch (K and V sides)
|   +-- attention_decode.rs         # Split-KV decode attention dispatch
|   +-- attention_decode_tests.rs   # Unit tests for attention decode dispatch
|   +-- attention_indexed.rs        # Decode attention over an explicit position list (port-local, qwen4_exp QSA, undispatched)
|   +-- attention_tq.rs             # Split-KV decode attention over TurboQuant-quantized KV (dense and indexed variants)
...
|       +-- attention_indexed.metal # attention_decode_partial over a position list (port-local)
|       +-- attention_tq.metal      # Decode attention over TurboQuant-quantized K/V (port-local)
