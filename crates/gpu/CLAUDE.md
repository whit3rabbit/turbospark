...
- `qsa_indexer.rs` + `shaders/qsa_indexer.metal`: `qwen4_exp`'s QSA (query-sparse attention) block indexer -- pooling and scoring (`qsa_pool_blocks_mean_fp16`, `qsa_score_blocks_fp16`; `docs/QWEN4_PHASE0.md` section 5, matched to `crates/compute/src/qsa_indexer.rs`'s CPU reference). Port-local; no reference backend kernel exists. **DISPATCHED from `families/qwen4/attn.rs` since 2026-09-05**: pooling through `encode_qsa_advance_blocks` whenever a block completes (every token, from position 0), scoring only above `index_top_k` complete blocks. Block SELECTION (top-k over `scores`) stays host-side, matching this port's existing MoE router precedent -- no kernel for it exists here, `turbospark_compute::qsa_indexer::select_blocks` is the whole of it. Norm (`rms_norm_centered`, per head) and RoPE (`rope_neox_subdim` at `rotary_dim = 64`, `theta = 1e7` -- the SAME object the trunk's own QSA attention already dispatches, per the CPU reference's resolved-from-source module doc) are NOT reimplemented here: both already exist as separate dispatches in this crate and are meant to be called once per head (query, at its own position) or once per block (pooled key, since blocks do not share a position) by whatever eventually wires this in.
  ...
  `qsa_score_blocks_fp16`: `scores[b] = sum_h(relu(sum_d(q[h*D+d] * pooled[b*D+d]))) / sqrt(D)`, one threadgroup per block, repeating the same two-stage SIMD-group reduction shape `ple_gate_fp16` uses for each query head. ReLU is applied to each head's dot product before the head scores are summed, matching section 5's `relu(q @ pooled^T).sum(over heads)` operation order. `tests/qsa_indexer_parity.rs::score_blocks_applies_relu_to_each_head_before_summing` pins the direction with a two-head fixture where the signed sum is negative while one head alone is strongly positive.
