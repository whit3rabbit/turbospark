...
/// Runs a generation to completion, returning the whole text at once.
pub(crate) async fn run_full(
    model: AppState,
    prompt_ids: Vec<foundation::TokenId>,
    config: GenerationConfig,
) -> Result<(String, RawDecodeResult), GenError> {
    ...
    match joined {
        ...
        Ok((Err(e), _)) => Err(GenError::Runtime(e)),
        Err(e) => Err(GenError::Join(e.to_string())),
    }
}

/// Runs a generation on the calling (blocking) thread, handing each decoded
/// piece of text to `on_delta` as it arrives. Both streaming endpoints and
/// [`run_full`] go through here, so the stop-string tail handling is written
/// once.
pub(crate) fn stream_blocking(
    ...
    model.with_producer(&mut |producer| {
        run_raw_completion(
            ...
            model.vocab_size(),
            |e| match e {
                RawDecodeProgress::Token { delta, .. } if !delta.is_empty() => on_delta(&delta),
                RawDecodeProgress::Tail(tail) if !tail.is_empty() => on_delta(&tail),
                _ => {}
            },
        )
    })
