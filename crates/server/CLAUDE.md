...
\-- tests/
    ...
    +-- harmony_channels.rs     # gpt-oss reasoning -> Anthropic thinking / OpenAI reasoning_content
...
8. **`StopReason::ToolCalls` is Gemma-only, so no fixture can produce it.** It fires on `tokenizer.tool_response_id`, and only Gemma's stop set contains that token (`dialect.rs`: Gemma is `[eos, eot, tool_response]`, ChatML is `[im_end, endoftext]`). The ChatML fixture can prove the decoder parses a call out of the stream, but the finish reason it reaches is `EndOfTurn`. Everything about the `tool_calls` finish reason belongs to the real-model gate:
   ```sh
