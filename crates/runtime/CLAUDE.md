...
- **`GptOss` has no flow yet and is refused at `open()` BY NAME** (ROADMAP M5). Its baseline, name table, MXFP4 routed pair and all four kernel-level differences (per-projection biases, attention sinks, YaRN rope scaling, the clamped SwiGLU) are landed and parity-tested; what is missing is `families/gptoss/`, which assembles them. The refusal names those four rather than falling through to a neighbour's flow, because every one of them produces fluent WRONG output rather than an error -- the failure mode `crates/runtime/CLAUDE.md` Gotcha 11 exists for.
