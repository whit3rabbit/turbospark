...
9. **A GGUF install's ROUTED blob layout is decided once at open; every other block type is read per tensor.** `RealForwardRunner` carries `routed_layout: RoutedBlobLayout` (affine, GGUF Q8_0, or GGUF Q4_K), set from `manifest.quant.routedExpert`, and the MoE dispatch sites go through `encode_moe_phase1_any` / `encode_moe_phase2_any` rather than branching individually, so a layout cannot disagree between two call sites and read one blob two ways. That is the only decision made once, because the blobs are uniform. Everything else keys on the resident entry's dtype tag at the dispatch site: `encode_gemv_any` and `encode_embed_any` both do, and they have to, because a real `Q4_K_M` mixes three block types in one file (Q4_K experts and embedding, Q8_0 attention, one Q6_K `output.weight`). A GGUF entry has NO scale or bias companions, so its `scale_offset` is zero and `offset - index_size` underflows: resolve companion offsets inside the affine arm, never before the branch.
