...
9. **A GGUF install's blob layout is decided ONCE at open, from the manifest.** `RealForwardRunner` carries `routed_gguf_q8_0`, set from `manifest.quant.routedExpert`, and the three MoE dispatch sites go through `encode_moe_phase1_any` / `encode_moe_phase2_any` rather than branching individually: phase 1 is dispatched from the cache-hit command buffer AND the main pass, and a layout that disagreed between them would read one blob two ways. The same decision reaches the embedding lookup through the resident entry's dtype instead, because that one is a property of a single tensor. A GGUF entry has NO scale or bias companions, so its `scale_offset` is zero and `offset - index_size` underflows: resolve companion offsets inside the affine arm, never before the branch.
