...
impl LogitProducer for RealForwardRunner {
    fn reset(&mut self) {
        ...
        // identify, and may later be parked in the session pool. Clearing
        // the map after the turn must not make those image-derived rows
        // eligible for token-prefix reuse.
