...
- **Upstream experiment inventory cross-reference.** The Swift original is
  ...
  |---|---|---|---|
  | Bounded `pread` beats `mmap` for cold experts (2.79 vs 9.88 ms) | production | `PreadExpertStreamer` is the production path, and upstream's COLD result reproduces here. `MappedExpertLayer` (2026-08-23) measures the case upstream did not: WARM and on the memory axis. Peak `phys_footprint` 3,652 -> 559 MiB and decode 53.8 -> 68.7 tok/s (1.28x) on the real Gemma 4 install at the `auto`-resolved 32 slots, re-measured 2026-08-29, output byte-identical against a pre-change binary as well as across the seam, because the slot cache holds a copy of bytes the GPU can read in place. Cold it is far worse exactly as upstream found (first prefill 74.8s against 2.5s). Both paths ship; `docs/EXPERT_RESIDENCY.md` | ported, and extended on an axis upstream did not measure |
...
- the head's KV was never PRIMED over the prompt, so a draft attended over
