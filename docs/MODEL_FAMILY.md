...
| Model Family / GGUF `general.architecture` | Key Architectural Features | `turbospark` (Rust) | `turbo-fieldfare` (Swift) | `llama.cpp` | `mlx-lm` | Measured Process Peak / RAM Notes |
| --- | --- | :---: | :---: | :---: | :---: | ---: |
...
| **Qwen3.8-Flash-Next / REAP-288 and Swift-1.5 GSQ-RCO IQ2_XS** (`qwen4_exp`, MLX and GGUF; catalog alias `qwen4exp-swift-iq2-xs`) | Fine-grained MoE (288-512 experts, top-10), GDN + sigmoid-gated norm, QSA block-sparse attention, PLE n-gram head, hyper-connections | **Full Support** (text-only; both artifacts gated separately) | *Planned* | Full Support (`qwen4exp`) | Not supported (absent from mlx-lm, checked 2026-09-08) | **~2.50 GiB REAP-288; ~1.60 GiB Swift IQ2_XS** (both M4 Max at 2,048 context; Swift at 16 slots. Process peaks, not general RAM requirements. See [frozen rows](BENCHMARKS.md#every-install-side-by-side).) |
...
| `QwenGdnMoe` (`qwen35moe`, upstream `qwen3_5_moe`) | **Verified (2026-09-19)** | The first MoE vision artifact (`mlx-community/Qwen3.6-35B-A3B-4bit`, whose bytes carry the depth-27 tower with `out_hidden_size` 2048) streams combined, reads a test page through the CLI and the server, and passes the four-stage mlx-vlm tower parity (cosine 0.99999+); a standalone MoE tower sidecar verifies via `pull-vision`. Chunked prefill with a vision map still refuses by name; a frozen vision memory oracle is not yet scheduled. |
