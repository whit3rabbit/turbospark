...
**QUALIFIED 2026-08-20, ON A DIFFERENT DRAFTER AND FAMILY.** That claim is this probe's, at ITS generation length, and the DFlash2 work found the limit it cannot see: byte-identity to a sequential decode holds for a few hundred tokens and then fails, because a batched verify row differs from a one-row decode pass in the last bits, so the streams part at the first near-tie. Nobody has re-run THIS probe long enough to say whether the same happens here; the mechanism is shared, so assume it does until measured. What survives exactly either way is that every BLOCK SIZE produces the same text.

**AND THE CAUSE NAMED HERE UNTIL 2026-08-21 WAS WRONG, in a way worth carrying because it is this page's own kind of error.** It said the two paths "run DIFFERENT KERNELS (`dequant_int4_gemm_simd` against `dequant_int4_gemv_simd`) whose accumulation orders differ" -- read off the two shaders rather than measured. Those two agree BIT-FOR-BIT at every batch width on data proven able to see a reassociation, against a positive control that differs on ~39% of the same outputs. What the divergence actually is: this port's own SHAPE FLOOR, 1e-5 nats with the argmax agreeing, against the 7.4e-6 that MLX's batched and cached passes differ by on the same architecture. Every engine has one; it is what the shape-floor arm of `scripts/kld.py` measures by running the REFERENCE twice. The differing-LOGIT COUNT is the wrong unit and reading it as the magnitude is what made an ordinary floor look like a defect. See `docs/DFLASH2.md`.
