...
- **Detailed TODO**:
  - [ ] **`qwen4_exp` (Qwen3.8-Flash-Next)**: 48 layers, fine-grained MoE (288 or 512 experts), GDN + gated attention, sigmoid-gated norm (not silu); see `docs/QWEN4_PHASE0.md` for the full config/tensor-layout fact-finding. **Intake landed 2026-08-31**: family/config surface, Phase 0 config gate, n-gram table classification and on-disk layout, streaming writer, manifest wiring -- see the Recent Landings bullet above. **Still open**: the `families/qwen4_exp/` decode flow itself (no forward pass wired), and the sigmoid-vs-silu fix to `gdn_gated_norm` this family's own Gotcha names but does not yet apply (`crates/gpu/CLAUDE.md` Gotcha 12). `ALLOWED_CACHE_SLOTS`'s cap of 32 slots holds only 6.25-11% of this family's expert table regardless of machine memory (AGENTS.md Gotcha 36), which bounds what "streams" can mean here before bring-up starts.
