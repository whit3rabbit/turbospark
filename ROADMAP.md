...
- **Test Suite**: Counts and gating conventions live in [docs/TESTING.md](docs/TESTING.md). A fresh `cargo test --workspace` passed on 2026-09-17 with no failures; this includes the two `turbospark-bench --test vision_sidecar_opener` cases, which now pass against the current 27-block synthetic tower. Formatting and the focused IG4 Rust, Swift package, and app checks are also green. Full Clippy and the broader Swift build remain separate gates.
