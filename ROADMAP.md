...
- **Test Suite**: Counts and gating conventions live in [docs/TESTING.md](docs/TESTING.md). Workspace build, tests with Metal, formatting, and Clippy passed on 2026-09-15 after the P3/P4 pass, with ONE known red: `turbospark-bench --test vision_sidecar_opener` (2 tests), the synthetic sidecar builder still writing a depth-2 tower against `SUPPORTED_VISION_DEPTH = 27` (2026-09-13's commit `146cfb3a`); the same collision was fixed in the runtime/server fixtures by the 2026-09-15 merges but this bench fixture remains. It is a fixture problem, not a runtime one -- real installs carry depth 27.
...
- **Artifact-blocked (2026-09-15)**: the pinned `qwen4-reap288.gturbo` install is no longer in either store (only `spark25.gturbo` remains under `~/.turbospark/models`; the `~/models` drift is worse than the artifact table's own warning -- MiniMax-M2, `qwen3moe`, `gptoss` and the vision sidecar are gone too, re-derived from `ls` this session). The decision rule stands for whoever re-pulls: interleaved `TURBOSPARK_QSA_FORCE_DENSE=1` vs unset on a >2048-token prompt, warmup discarded, three pairs; if the delta is within a few percent of decode, record the negative in `docs/QWEN4_EXP.md` and move this item to Do Not Revisit; build the kernel only if the commit share clearly exceeds the router's 0.13 ms/token, and then remove the whole round trip (kernel-written sorted position list, per-QSA-layer position buffers per the `attn.rs:426-433` safety note), not just the sort.
- **Files to Touch / Create**:
...
- **Sequence**:
  ...
  - [x] **IG2**: Deliver a complete quantized install and staged CLI pipeline with PNG output, metadata, progress, and cancellation.
    - **Closed-gate follow-up**: The bounded trace and complete packed gates are recorded below. App work remains deferred until IG3.
    ...
    - **Current gate status (2026-09-16)**:
      ...
      3. **Real install and catalog path**: local `turbospark-model pull-image` records a separate image install and does not create a text `Mlx` row. The image catalog pins the canonical Diffusers source, its live rot guard passes, and the real `pull-image --repo OWNER/NAME@REV` exercise streamed the source into a verified atomic install with matching packed payload hashes.
  ...
  - [ ] **IG4**: Expose the same runtime through Swift; add chat image mode, job serialization, preview/save/regeneration, and profile-scoped artifact persistence.
    - **Implementation progress (2026-09-17)**: Added the separate `TsImageSession` C ABI and Swift `TurboSparkImageSession`, including stage callbacks, cancellation, explicit PNG ownership, and camelCase metadata. The app now has an image-mode composer toggle, an explicit image-install path override, a process-wide FIFO image-job coordinator, live progress, preview, save, regenerate, and PNG artifacts under `AppStorageRoot` for the active profile. Saved image paths render in persisted transcript rows as well as the artifact panel. Interrupted jobs remain transient and are not restored as completed work. Focused Rust ABI tests, the real-install Swift cancellation path, and Swift package/app builds pass. The default nine-step real-install PNG run remains incomplete, and the measurement/validation backlog below remains release gates before checking IG4.
...
- **Still blocked on downloads, unscheduled**: museglimmer, qwen3moe, qwen4-reap288, minimax (their gate files exist; the installs do not). qwen36's clause lives in P4.1's descope note.
