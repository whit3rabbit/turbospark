...
- **Architectures**: 15 declared `ModelFamily` variants. The fifteenth,
  ...
  `verified`. Text and image intake: the deepstack vision half landed
  2026-09-19 (the Vision bullet below; `DEVIATIONS.md`'s `qwen3_vl`
  section). Dense `qwen3` GGUF execution is verified on 0.6B Q8_0. Dense Qwen2/Qwen2.5 runs three real artifacts end to end: the MLX/HF 4-bit install and both single-file GGUF conversions (the pinned official Q3_K_M on the Q3_K resident kernels, and a single-file Q4_K_M), each with frozen quality and memory rows. MiniMax-M2 GGUF execution is implemented, but repetitive low-temperature smokes block release and catalog promotion. `deepseekV4Flash` remains scaffolded. The full support matrix lives in [docs/MODEL_FAMILY.md](docs/MODEL_FAMILY.md).
...
#### 2. Dense `qwen2` / `qwen2.5` Validation and Format Expansion -- LANDED, Q3_K kernels built, RELEASED 2026-09-19
...
- **RELEASED (2026-09-19): the Q3_K resident kernels are built and both GGUF artifacts are gated.** The kernel set is a resident GEMV plus an embedding lookup, no routed pair -- the Q6_K footing, since every real Q3_K file in this port's orbit is dense. The CPU reference (`compute::quant_gguf::q3_k`) is anchored to ggml itself by a generated oracle (`scripts/ggml_q3_k_oracle.c` against brew libggml: ggml quantizes the bytes and decodes the floats, the Rust decoder is the only side under test), the Metal kernel is held to the reference by `crates/gpu/tests/dequant_q3_k_gemv_parity.rs` with a mutation-checked case per trap, and the block layout carries three of them worth restating: the f16 super-scale is the LAST field of the 110-byte superblock, the high-bit sign run is indexed by `e % 32` with the BIT chosen by `e / 32`, and the sixteen 6-bit scales are shuffled out of twelve bytes through ggml's four-word aux trick with a cleared high bit SUBTRACTING 4 from the stored 2-bit level. The dtype tag is 24 in the resident-tag space (ggml's own Q3_K id 11 was taken by IQ4_NL), wired through `resident_writer`, `EXECUTABLE_GGUF_TYPES`, `GGUF_BLOCK_DTYPES` and `EXECUTABLE_GGUF_DTYPES`, with a synthetic mixed install opening and decoding in `gguf_install_refused.rs`.
- **The pinned official Q3_K_M GGUF (the roadmap's named artifact) now executes**: streamed into `qwen25-7b-q3km.gturbo`, greedy and sampled CLI smokes coherent with EndOfTurn, quality gate frozen at perplexity `12.2878` with stable greedy and sampled digests and the 8-slot constrained digest equal to the 16-slot one, memory oracle at `651 MiB` peak at 8192 context with `+0.02 MiB` replay and all three protocol cases stopping endOfTurn (readings on battery; AC noted for the row). Its census -- Q3_K on every attention and FFN projection, Q4_K on the embedding table, Q6_K on the head, three Q5_K tensors -- makes it the first mixed Q3_K/Q5_K install this port runs.
- **The second real artifact is a single-file Q4_K_M.** The official repo splits its Q4_K_M and Q8_0, and split GGUF stays out of scope for the dense qwen2 walk, so `mradermacher/Qwen2.5-7B-Instruct-GGUF`'s single-file Q4_K_M (same base checkpoint) streams as `qwen25-7b-q4km.gturbo`: smokes coherent, quality frozen at perplexity `11.6563` with stable digests and `1.00x` at 8 slots, memory oracle frozen. Both installs were REMOVED after their gates froze, per the session's space goal; re-pull with `turbospark-model pull`.
- **Still outside, each its own validation**: larger Qwen2 checkpoints (the 72B line), Qwen2-MoE, Qwen2-VL, split GGUF, and native unquantized BF16/FP16 safetensors conversion. Q3_K routed experts fail at the routed dispatch by name (no routed pair), the same weaker footing Q6_K and Q5_K stand on.
- **Files Touched / Created**:
  - `crates/compute/src/quant_gguf/q3_k.rs` [NEW], `crates/gpu/src/shaders/dequant_q3_k.metal` [NEW], `crates/gpu/src/dequant_q3_k_gemv.rs` [NEW]
  - `scripts/ggml_q3_k_oracle.c` [NEW], `crates/compute/tests/generated/quant_gguf_q3_k_oracle.rs` [NEW], `crates/gpu/tests/dequant_q3_k_gemv_parity.rs` [NEW]
  - `crates/bench/tests/` (Qwen2 memory and quality gates, GGUF artifact rows)
  - `crates/catalog/src/models.json` (`qwen25-7b-q3km`, `qwen25-7b-q4km`)
  - `crates/runtime/tests/gguf_install_refused.rs`, `crates/repack/tests/qwen2_checkpoint_network.rs`
  - `docs/MODEL_FAMILY.md`, `docs/TESTING.md`, `docs/GTURBO.md`, `DEVIATIONS.md`
...
- **Landed (2026-09-19): the GLM and Kimi K2 dialects, derived from real tables.** Both are registered `ChatDialect`s with resolvers, native text-marker decoder arms, and parsers:
  - **GLM** was read off `zai-org/GLM-4.7-Flash`'s own `tokenizer.json` and `chat_template.jinja`: the frame is the special pair `<|user|>` / `<|assistant|>` with `<|observation|>` as the tool-result turn marker AND the third generation stop (`generation_config.json` ends a turn at `<|endoftext|>`, `<|user|>` or `<|observation|>`); the generation prompt forces the think frame open (`<|assistant|><think>`, the Spark arrangement); and the tool markup (`<tool_call>` 154843 through `</arg_value>` 154850) is ADDED but NOT SPECIAL, so it survives detokenization as text -- the decoder arm is a DeepSeek-DSML-shaped text scan, and `GlmToolCallParser` reads string argument values RAW and everything else JSON, exactly the template's own `tojson` rule.
  - **Kimi K2** was read off `moonshotai/Kimi-K2.5`'s `tokenizer_config.json` added-token list and template (the K2 line ships no `tokenizer.json` at all -- it is tiktoken-only, checked across K2, K2.5, K2.6, K2.7-Code, Thinking and the mlx/ISTA/nvidia builds): `<|im_end|>` is SPECIAL and ends every turn, which is why the Kimi probe (`<|im_assistant|>` + the Kimi-owned `<|im_middle|>`) is tested BEFORE the ChatML arm -- a K2 table carries ChatML's whole witness; the tool markers (163595-163599) are ADDED but NOT SPECIAL like GLM's, so the arm is a text scan; `KimiToolCallParser` keeps the checkpoint's own `functions.NAME:IDX` id verbatim and cuts the name out of it.
  - **Fixtures and tests**: `GlmTokenizer` is reduced from the real table (all 36 added tokens with real ids, real byte-level vocab entries); the `KimiK2Tokenizer` fixture carries the real added-token list over a disclosed synthetic BPE body. Both dialect files pin resolution BY NAME (the `tokenizers` library renumbers sparse fixtures; real installs are dense). The decoder tests cover prose around calls, marker splits across flushed-text deltas, forced-open think frames, two-call turns, unknown tools, stray closes and truncations; the dialect-support link table feeds each its own markup; the partial-marker withholding is mutation-checked.
  - **Witness status -- NOT closed by this work.** For GLM, the dialect's own checkpoint is real and reachable (`unsloth/GLM-4.7-Flash-GGUF` probes `general.architecture = deepseek2`, 844 tensors) but is gated at deepseek2 intake by four recorded descopes the header evidence now pins: `q_lora_rank = 768 > 0` (the trio `attn_q_a`/`attn_q_a_norm`/`attn_q_b` is in the file), SPLIT `attn_k_b`/`attn_v_b` (no fused `attn_kv_b` anywhere in the file), noaux_tc routing (`exp_probs_b.bias`, top-4, renorm, scale 1.8), and MXFP4 expert tensors (the 16.97 GB `MXFP4_MOE` build's 138 expert tensors; the Q4_K_M build is refused on Q3_K_M experts with no routed arm, and Q8_0 is 31.84 GB). For Kimi, the witness is blocked twice over: no `tokenizer.json` exists anywhere in the ecosystem to source the required sidecar from, and every K2 checkpoint is 1T-class, past this machine at any quantization. The dialect tier's honest claim is the real-table fixture evidence above; a native decoder's first real generation smoke remains owed for both.
- **Files to Touch**:
...
- **Sequence**:
  ...
  - [x] **IG4**: Expose the same runtime through Swift; add the top-level Images destination, job serialization, preview/save/regeneration, and profile-scoped artifact persistence.
    ...
    - **Image workspace (2026-09-19)**: Create and Organize share a gallery with prompt reuse, export, quick Trash actions, and missing-file recovery. Create has a bottom composer with settings below it, separate model/quantization selection, and one image by default. The 1-4 image option runs serialized batch-one requests with successive seeds and saves each output before advancing. Organize adds search and bulk removal. The native size remains the selected install's fixed envelope (1024-by-1024 for Z-Image Turbo).
  - [ ] **IG5**: Tune measured bottlenecks without weakening quality or memory gates; approximation work needs a separate proposal.
- **First Release Boundary**: Native batch one, with optional sequential 1-4 image requests in the app, native Metal, and exact dense execution with validated quantization. Image editing, LoRA, native batching, agent tools, HTTP image endpoints, PISA, and approximate timestep reuse are deferred. Existing text consumers must retain their behavior.
...
- **Honest boundary**: end-to-end speculation on Bonsai/Ternary still needs a DRAFTER artifact (mlx conversions drop `mtp.*`; no DFlash2 state in those installs), so the real-model int2 batched arm runs through `produce_batched` only where a drafter exists. The 1-bit synthetic verify gate covers the kernel end to end on Metal; a real ternary `produce_batched` arm stays gated on a drafter (the probe refuses a headless install at open, by design). **Re-confirmed 2026-09-19 on the Bonsai-2 line** (`prism-ml/Ternary-Bonsai-2-27B`): the conversion declares `mtp_num_hidden_layers: 0` / `components.mtp: false` and the GGUF repo carries no drafter either, so the reopening condition is still just "a drafter artifact exists". A folded trunk with an open drafter is now additionally refused at open (`docs/BONSAI2.md`), since the drafter's own weight inputs would need the same transforms.
...
- **Bonsai-2 DONE (2026-09-19)**: `prism-ml/Ternary-Bonsai-2-27B-mlx-2bit` -- the HADAMARD-FOLDED line, a fourth checkpoint of the dense `qwen3_5` architecture in a rotated weight basis -- brought up end to end; `docs/BONSAI2.md` is the contract page. Support = activation-side transforms (signed block-FWHT at 7 call sites per layer pair + embedding inverse + head), the manifest `hadamard` section + `hadamard.bin`, a BF16 resident-matrix GEMV for the checkpoint's F32-shipped `in_proj_a/b`, and the contract-carrying walk (`write_qwen_gdn_dense_install_streamed_with_hadamard`, auto-routed by the catalog streamer). `bonsai2_{quality_gate,memory_oracle}.rs` [NEW] frozen from first runs. Perplexity **5.8134** -- between qwen38's 4-bit 4.9432 and first-gen ternary's 2-bit 6.8350 on the same corpus, which is prism's 98.2%-retention claim landing AND the proof the transforms are right (a missing transform reads in the hundreds or thousands, never 5.8); peak **660 MiB**, the fourth independent reading of the dense-qwen35 ~660 row (the transform plan added nothing to the count); 12.0-13.2 tok/s, the ternary band. Both CLI smokes coherent. The GPU kernels are parity-tested against `compute::kv_quant`'s rht pair and mutation-checked.
...
- **Owed on the pairing**: DONE 2026-09-19/20, closed in two halves after the harness's own preflight refused the tail mid-session (a concurrent Swift build started; the refusal is the system working). Ternary's `COOLING=max` arm: all-Nominal, short decode ~3.35 J/token at ~45.7 W / 13.35 tok/s, pairs to 1.4%. The tail (2026-09-20, one thermal condition): qwen38 `COOLING=max` ~1.74 J/token long-decode / 1.525 short-decode at ~21 tok/s, and bonsai2's FIRST ROW -- all eighteen rows Nominal, nothing over 5% spread, short decode 2.4372 J/token at 14.05 tok/s reproducing to 0.03%. **The clean same-architecture pairing reads 1.60x J/token at 67% of throughput for 2-bit against 4-bit** (`docs/POWER_BASELINE.md`), replacing every thermally-asterisked cross-width sentence on that page. `ornith9b` and `ornith35b` remain MISSING FROM DISK and block their rows.
...
| `qwen38-27b.gturbo` | 14G | RE-PULLED 2026-09-19 (walk verified against pinned constants). Backs `qwen38_{quality_gate,memory_oracle}`, the qwen38 KL row (P4.1, closed), steering probes; power rows: 2026-09-16 uncooled AND 2026-09-20 COOLING=max (P4.3, `docs/POWER_BASELINE.md`) |
...
| `bonsai2.gturbo` | 7.6G (`~/.turbospark/models/text/`) | STREAMED 2026-09-19. `bonsai2_{quality_gate,memory_oracle}` [NEW files, P4.2] frozen and asserted: perplexity 5.8134, peak 660 MiB; first power row CAPTURED 2026-09-20, COOLING=max (P4.3). The Hadamard-folded Bonsai-2 line (`docs/BONSAI2.md`) |
...
| `qwen25-7b-q3km.gturbo` | 3.5G | STREAMED and GATED 2026-09-19 (Q3_K kernels; quality 12.2878 + digests, oracle 651 MiB), then REMOVED per the session's space goal; re-pull `qwen25-7b-q3km` |
| `qwen25-7b-q4km.gturbo` | 4.4G | STREAMED and GATED 2026-09-19 (quality 11.6563 + digests, oracle frozen), then REMOVED per the session's space goal; re-pull `qwen25-7b-q4km` |
