...
| **MoE Speculative Refusal Strings** | DONE 2026-08-25 (`d78f6b3`) | Re-checked 2026-09-01: `mtp_state.rs`, `dflash_state.rs` and `speculation_policy.rs` already frame the MoE case as "the batched routed verify itself runs, so this is a checkpoint gap and not a missing kernel"; CLI and server both read the string from `RealForwardRunner::speculation_blocker()` rather than duplicating it. A repo-wide grep for the old "dense-only" / "no batched kernel" refusal phrasing found zero live occurrences. This row was stale, not the code. |
...
3. **MoE Drafter & Speculation Cleanup**:
   - ~~Fix stale refusal strings in `mtp_state.rs`, `dflash_state.rs`, `speculation_policy.rs`~~ DONE 2026-08-25 (`d78f6b3`); re-verified 2026-09-01, see Active Tasks table.
...
- **Detailed TODO**:
  ...
  - [x] **Speculation String Cleanup**: DONE 2026-08-25 (`d78f6b3`), re-verified 2026-09-01. `mtp_state.rs`, `dflash_state.rs`, `speculation_policy.rs` and CLI session error reporting (`crates/cli/src/generate/session.rs`, mirrored in `crates/server/src/real_model.rs`) all state the MoE case correctly: the batched routed verify runs, and what's missing is an ingestible drafter for that architecture, not a kernel.
