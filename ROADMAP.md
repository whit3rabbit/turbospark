...
- **Sequence**:
  ...
  - [ ] **IG2**: Deliver a complete quantized install and staged CLI pipeline with PNG output, metadata, progress, and cancellation.
    - **Next owner**: Build or adapt one pinned real source into a complete image install, then run the packed native parity and quiet-machine gates. App work remains deferred until IG3.
    ...
    - **Progress (2026-09-14)**: The tiled linear Metal launch now matches the shader's 32-column threadgroup stride, and the focused packed linear parity test passes on real Metal. Image operations now defer command-buffer waits until CPU read seams while retaining the producing pass for output lifetime; the caption-refiner result is reused between denoise steps. A full packed native parity rerun against the pinned local install still produced no first callback after roughly twelve minutes and was stopped without a numerical result. The remaining performance work is to reduce operation-level command-buffer boundaries, likely by batching or fusing transformer-block work, before rerunning the production gates.
    - **Current gate status (2026-09-13)**:
      ...
      2. **Packed-runtime parity**: the opt-in gate compares conditioning, all nine latent updates, final latents, and decoded output against the IG1 fixtures. It has not passed on a complete pinned packed install in this checkout.
      ...
      5. **Resource and quality closure**: the packed PNG, quality, and resource-oracle tests exist and record latency, nine forwards, peak `phys_footprint`, allocations, retained buffers, physical reads, and swap. They remain unrun without a complete packed install on a quiet machine.
