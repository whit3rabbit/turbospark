...
- **Sequence**:
  ...
  - [ ] **IG2**: Deliver a complete quantized install and staged CLI pipeline with PNG output, metadata, progress, and cancellation.
    - **Next owner**: Continue from the pinned packed install failure, isolate the first divergent denoise intermediate, then rerun the complete packed gates. App work remains deferred until IG3.
    ...
    - **Current gate status (2026-09-13)**:
      ...
      2. **Packed-runtime parity**: the opt-in gate compares conditioning, all nine latent updates, final latents, and decoded output against the IG1 fixtures. Conditioning completes, but both complete pinned-install runs fail at rollout step 1, with relative L2 about 1.414 versus the frozen 0.923 quality envelope. It has not passed on a complete pinned packed install in this checkout.
    ...
    - **Next-LLM execution order**: first add or use a bounded first-step intermediate trace, comparing conditioning, patched latent, noise-refiner output, main-transformer output, velocity, and scheduler update against the frozen IG0 arrays. Second, determine whether the 1.414 error is a tensor-source/layout issue, a BF16-versus-F32 reference boundary, or a packed/native execution issue. Third, make the smallest scoped fix and rerun the focused boundary test. Fourth, rerun the complete quality/VAE gate, then the cancellation and PNG gates, and finally the quiet-machine resource oracle. Stop and record a new evidence boundary if any complete run fails before the next check; do not loosen tolerances or start IG4 from an unresolved packed quality failure.
