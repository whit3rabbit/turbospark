...
| **Prefill Energy Capture** | UNBLOCKED 2026-09-05. ~12 min, sudo, quiet machine, AC. | Was BLOCKED on a bench gap rather than on a measurement. Attempted 2026-09-05: `scripts/power.sh` drives `turbospark-bench`, and `crates/bench/src/real_model.rs` calls `run_raw_completion` directly -- it never reaches `run_raw_completion_chunked`, so chunking, `TURBOSPARK_ROUTED_BATCH` and `TURBOSPARK_BATCHED_GEMV` are all invisible to it. A `power.sh` run today would silently re-measure the old sequential path and read as a chunked-prefill row without being one. Needs `crates/bench`'s model mode taught to call the chunked path (mirroring the CLI's own `generate` loop) before this is measurable at all; that is new scope, not a 12-minute capture. **THAT SCOPE LANDED 2026-09-05** (`30e184d`): `turbospark-bench --prefill-chunk off|auto|N`, default OFF so no frozen row moves, plus a `seq|chunked` arm pair in `scripts/power.sh`. Only `TURBOSPARK_PREFILL_CHUNK` needed wiring; `TURBOSPARK_ROUTED_BATCH` and `TURBOSPARK_BATCHED_GEMV` are read INSIDE the runtime's chunk drivers, so they go live the moment the driver is engaged and got a header echo rather than a flag. **So this row is no longer blocked, it is simply OWED**: `ARMS=seq,chunked scripts/power.sh` on AC, quiet machine, ~12 min. Read `rows.tsv`'s per-arm dispersion and the contamination floor before believing any J/tok. |
...
1. **PF-02 Completion** (family widening and Step 5's MXFP4 arm are DONE; what is left is measurement plus one kernel):
   ...
   - ~~Teach `crates/bench` to reach the chunked path~~ DONE 2026-09-05 (`30e184d`): `--prefill-chunk off|auto|N`, default OFF, plus a `seq|chunked` arm in `scripts/power.sh`. This is what unblocks the two runs below.
   ...
   - **Read one `TURBOSPARK_PHASES=1` run's `pread` bucket on `qwen4-reap288`**, short prompt so the divisor is decode (AGENTS.md Gotcha 21). This is the cheap test of the standing hypothesis that this family's prefill is pread-bound and therefore that chunking cannot pay on it; it is NOT another A/B, and `TURBOSPARK_DISPATCH_PROFILE` is the wrong instrument (Gotcha 66).
...
- **Detailed TODO**:
  ...
  - [ ] **Step 6 Throughput A/B**: re-run owed. Attempted 2026-09-05 and contaminated (see the Active Tasks row). It can now go through `turbospark-bench --prefill-chunk auto` with `TURBOSPARK_BATCHED_GEMV=1` exported, rather than the hand-computed CLI footer arithmetic that attempt used, and the bench header echoes the seam so the arm cannot silently be the wrong one.
