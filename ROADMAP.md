...
- **Sequence**:
  ...
  - [ ] **IG2**: Deliver a complete quantized install and staged CLI pipeline with PNG output, metadata, progress, and cancellation.
    ...
    - **Current gate status (2026-09-13)**:
      ...
      5. **Resource and quality closure**: the complete packed quality/VAE gate and PNG metadata gate pass. The quiet resource oracle has a valid cold report and a warm-only rerun in progress; its cold peak is recorded separately from the smaller IG0 reference ceiling because the packed VAE reaches `20,725,728,336` bytes. Dedicated real-install cancellation passes. No-device execution remains unverified because the cross-target toolchain is unavailable.
