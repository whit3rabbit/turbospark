...
class PowerPreflightTests(unittest.TestCase):
    def run_preflight(self, process, cooling="auto", max_status="0", output="explicit"):
    ...
    def test_model_processes_refused_before_capture(self):
        for name in (
            ...
            with self.subTest(process=name):
                ...
                result, created, _, _, _, _ = self.run_preflight(process)
    ...
    def test_partial_fan_pin_failure_restores_automatic_curve(self):
        result, created, trace, _, _, _ = self.run_preflight(
    ...
    def test_default_output_is_private_and_unpredictable(self):
        result, created, _, modes, rows, _ = self.run_preflight("", output="default")
        self.assertEqual(result.returncode, 2)
        self.assertIn("SUDO_SENTINEL", result.stderr)
        self.assertFalse(created)
    ...
    def test_world_accessible_explicit_output_is_refused(self):
        result, created, _, _, _, rows_created = self.run_preflight("", output="unsafe")
        self.assertEqual(result.returncode, 2)
        self.assertTrue(created)
