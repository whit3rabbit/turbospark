# Archived module guide: selection

This is the detailed module guide moved from `crates/selection/AGENTS.md`. The active entry point is the short guide at `crates/selection/AGENTS.md`; this page preserves the deeper directory map, commands, gotchas, and evidence notes for on-demand reading.
...
|   +-- distribution.rs     # ShapedDistribution & speculative residual rejection sampler
...
\-- tests/
    ...
    +-- min_p.rs            # Min-p truncation threshold and composition-order tests
    +-- penalty.rs          # Repetition penalty calculation unit tests
    +-- presence_frequency.rs# Presence/frequency penalty generated-suffix-only tests
    +-- rank_top_k.rs       # Top-k partial ranking vs full sort equivalence tests
    +-- rejection_sampling.rs# Speculative rejection sampling distribution tests
...
- `distribution.rs`: `ShapedDistribution` representing a shaped probability distribution over an explicit support, and speculative residual sampling (`sample_residual`).
...
8. **`presence_penalty`/`frequency_penalty` see only the GENERATED suffix of `history`, never the prompt, and `min_p` composes AFTER the top-k cut.** Added 2026-08-30 (`ShapingConfig::with_presence_penalty`/`with_frequency_penalty`/`with_min_p`, `penalty::apply_presence_and_frequency_penalty`, `truncation::truncate_by_min_p`). `choose.rs::select` slices `history` at `history.len() - generated` where `generated = (position as usize).min(history.len())` -- `position` already doubles as the generated-token count for every caller in `crates/runtime` (documented on `select`'s own doc comment), so this needed no new parameter. That is llama.cpp's convention, not OpenAI's: upstream OpenAI penalizes the WHOLE context including the prompt, so a long prompt with a coincidentally repeated word is not itself a trigger here the way it is there. Min-p's threshold is read off `ranked[0]` (the ranked prefix's true global max, whether or not `top_k` pre-limited the ranking) and is applied to whatever `top_k`/`top_p` left standing, never able to reintroduce a candidate an earlier stage already excluded (`tests/min_p.rs`'s composition-order test). Both penalty builders validate `[-2, 2]` (OpenAI's own bound); `with_min_p` validates `[0, 1)` and treats `0.0` as "disabled" (`None`) rather than as a no-op filter, so `min_p()` returning `None` and returning `Some(0.0)` are NOT the same thing to check for downstream. See `crates/server/AGENTS.md`'s matching Gotcha for the wire-level side of this (the `/v1/chat/completions` vs `/v1/completions` request-shape asymmetry, and why `turbospark-invocation` does not gain matching flags).
