# Archived module guide: repack

This is the detailed module guide moved from `crates/repack/AGENTS.md`. The active entry point is the short guide at `crates/repack/AGENTS.md`; this page preserves the deeper directory map, commands, gotchas, and evidence notes for on-demand reading.
...
|   |   +-- ngram.rs                # `qwen4_exp` n-gram PLE table: writer + streamed ingest
|   |   +-- orchestrate.rs          # Repack orchestration & layer planning
|   |   +-- shards.rs               # Multi-shard reader & classification
|   |   \-- vision.rs               # The qwen3_5 vision tower's ingest: the walk's one TWO-DESTINATION arm
...
- `gemma4_checkpoint/ngram.rs`: `qwen4_exp`'s hashed n-gram PLE table (`NgramTableSpec`, `NgramTableWriter`, `NgramPlan`, `write_ngram_table`), and **the walk's only STREAMED-READ arm**: at 32 GB the table cannot be held in memory at all, where the MTP head's and the vision tower's bytes both are. `orchestrate_gemma4_checkpoint_sharded` therefore does not read it -- it classifies the table's names into `ClassifiedNames::ngram_shards`/`ngram_meta` and carries them as an owned `Gemma4RepackOutput::ngram: Option<NgramPlan>` (names only, never bytes). The actual shard-by-shard read-then-write happens in `write_ngram_table`, called directly from BOTH `write_gemma4_install` and `write_gemma4_install_streamed` in `mod.rs` -- the same "both writers" shape `mtp.rs`'s Gotcha above describes, for a different structural reason: the head's arm was duplicated because its non-streamed home could not see the streamed path, while this one is duplicated because NEITHER writer's non-streamed data structure (`Gemma4RepackOutput`) can hold what it would need to carry.

  **THE `.scales`/`.biases` COMPANION SKIP IN `classify_all` ALSO ATE THE TABLE'S OWN SCALE AND BIAS PLANES, AND ONLY A FIXTURE THAT BUILT ALL THREE PLANES COULD SEE IT.** `classify_all`'s loop drops any name ending `.scales`/`.biases` before classification, because a trunk tensor's companions are read alongside its `.weight` by `pass_through_packed` and must never become independent resident entries. The n-gram table's shard tensors are role-named `weight`/`scales`/`biases` (`Gemma4Bucket::NgramShard`), so its `scales`/`biases` planes matched that same suffix and were silently dropped before `classify_for_family` ever saw them -- `write_ngram_table` then failed every shard with `MissingTensor("... scales")`. The exemption is `!name.contains(NGRAM_CONTAINER)` added to the skip condition. `both_writers_carry_the_ngram_table` (`tests/qwen4_ngram_install.rs`) is what caught it, and it is the fixture-before-download instance of Gotcha 8's rule one gate earlier than usual: the bug never reached a real writer at all, because the OLD code refused every n-gram tensor by name before this skip could matter, so nothing before this fixture had ever asked whether the skip was even scoped correctly.

  `synthetic_qwen/qwen4.rs`'s fixture deliberately builds only ONE ordinary layer plus a PLE layer carrying nothing but the table -- it exists to prove the writer wiring, not `qwen4_exp`'s hyper-connection/GDN/QSA decode flow (a later phase's fixture). Its n-gram bytes are deterministic filler rather than valid quantized data, matching `NgramTableWriter`'s own contract that it copies planes VERBATIM and reads no value; what `the_installed_table_places_each_shard_at_its_own_rows` checks is PLACEMENT (each shard's weight plane is filled with its own shard index), the same property `tests/qwen4_ngram_store.rs` checks for the writer in isolation.
- `gemma4_checkpoint/vision.rs`: the `qwen3_5` vision tower's ingest (ROADMAP M-V3), and **the walk's only arm with TWO destinations**. Every other name group lands entirely in the resident index (`mtp.*`, `dflash.*`) or entirely in packed blobs (the routed experts); the tower splits its 27 blocks into `packed_vision/blobs.bin` and its 9 non-block tensors (`patch_embed.*`, `pos_embed`, `merger.*`) into `model_weights.bin`. That is why it is read BESIDE the drafter arms rather than beside the `write_packed_vision` call: `resident.entries` is consumed a few lines below, and reading the tower after that point writes one whose merger and patch embedding are simply missing -- an install that validates, opens, and cannot turn an image into tokens. The blocks reuse `PackedExpertsLayout` VERBATIM, which is not a pun: `StreamLayout` interprets nothing, so a "layer" is a file and an "expert" is a fixed-stride blob inside it, and a double-buffered block loop wants exactly that -- the tower is one layer of `depth` experts and gets `PreadExpertStreamer`, `ExpertCache` and the read pool with no new I/O code. `model_io::load_packed_layout_from` and `StreamLayout::from_packed_layer_in` take the subdirectory as a PARAMETER for this, rather than growing a second copy of the stride fallback and the missing-entry check that Gotcha 2 is already about one of. **It is also the walk's only FP16 destination** (Gotcha 12).
...
12. **A COMPONENT'S CONFIG SAYS WHAT THE ARCHITECTURE HAS; ONLY THE TENSORS SAY WHAT THE ARTIFACT SHIPS -- AND FOR THE VISION TOWER THE TWO DISAGREE ON A PUBLISHED CHECKPOINT.** ROADMAP M-V3 ingests `vision_tower.*` for the `qwen3_5` family. FIVE published checkpoints declare a `vision_config`, and all five declare the IDENTICAL tower: depth 27, hidden 1152, intermediate 4304 (**not** the 4608 the planning table guessed by analogy with the merger's width), `num_position_embeddings` 2304, `mrope_section` [11, 11, 10] and the same four token ids -- `Qwen/Qwen3.8-27B`, `prism-ml/Bonsai-27B-mlx-1bit`, `prism-ml/Ternary-Bonsai-27B-mlx-2bit`, `ornith-ai/Ornith-1.5-35B-A3B` and `-9B`, read off the published files rather than inferred. **`ornith-ai/Ornith-1.5-35B-A3B-MLX-4bit` ships NO `vision_tower.` tensor at all.** It is `ornith_config.rs`'s own "DECLARED AND UNSHIPPED" note, one component over from `mtp.*`.
    ...
    **`ArchConfig.vision` describes the INSTALL, not the checkpoint**, so `parse_qwen_gdn_dense_config` deliberately leaves it `NONE` and `parse_vision_config` is a SEPARATE parse the install driver calls. Folding it in would compare 27 against the 0 every existing install declares -- AGENTS.md Gotcha 24 exactly, refusing every qwen35 and qwen36 install ever written -- and would redden `every_published_checkpoint_parses_to_one_baseline` and both Ornith cases, which assert `derived == baseline` on the whole struct.
    **`vision_should_ingest` is a CONJUNCTION** of the caller's request (`arch.vision.is_active()`) and the artifact's answer (`!vision_bases.is_empty()`), and the terms fail in opposite directions: the first keeps every existing text-only walk byte-identical, the second stops a correct request from writing a manifest that claims a tower whose `packed_vision/` files do not exist, which would stop a WORKING install from opening.
    **`vision_arch_for_manifest` zeroes the declaration when nothing was written**, so the manifest states what the install HAS. That is the MTP head's rule -- whether an install carries a drafter is answered by whether `mtp.fc.weight` is in the resident index, with no flag that could disagree with the bytes -- applied to a component that does have a manifest field.

13. **THE TOWER IS THE ONE COMPONENT HELD AT FP16, WHICH IS `narrow_raw_to_bf16`'s RULE POINTING THE OTHER WAY -- AND THE TWO PUBLISHED CHECKPOINTS DISAGREE ABOUT THE SOURCE DTYPE.** Gotcha 9 narrows every unquantized TEXT tensor to BF16 because BF16 is the only unquantized width the text kernels dispatch. M-V2's vision kernels bind `half` and are alone in `crates/gpu` in doing so, so `convert_raw_to_fp16` exists beside it and sending the tower through the BF16 narrowing would throw away three mantissa bits to store a precision nothing then reads.
    **Both arms are needed and a fixture only reaches one.** `prism-ml/Bonsai-27B-mlx-1bit` ships the tower F16 (verbatim copy); `mlx-community/Qwen3.8-27B-4bit`, the artifact the real M-V3 gate streams, ships the SAME 333 tensors at the SAME shapes in **BF16** (a real conversion). A walk handling F16 alone passes every fixture built from the 1-bit file and refuses the very checkpoint the milestone exists to ingest. The synthetic tower is F16 like the dense fixture it hangs off, so `a_bf16_tower_converts_to_fp16_exactly_in_the_normal_range` covers the other arm offline.
    **BF16 to FP16 is EXACT in FP16's normal range, measured rather than argued**: the mantissa WIDENS (7 bits to 10) so it cannot round, and all 7,680 BF16 values in [2^-14, 65504] round-trip bit-for-bit. Only the EXPONENT can fail, and the two ends are treated differently on purpose. Above 65504 the value is **REFUSED by name, never clamped**: BF16 reaches 3.4e38, and an `inf` weight is AGENTS.md Gotcha 59's failure mode -- it does not crash, it becomes NaN in the activations, and NaN then reads as a PERFECT score on every rank and top-k instrument downstream. Below FP16's smallest normal it degrades into subnormals and is COUNTED, because a weight that small contributes nothing a norm can see and refusing one would reject a checkpoint over a value it does not use.
    **MEASURED ON THE REAL CHECKPOINT** (`mlx-community/Qwen3.8-27B-4bit`, 2026-08-27): 333 tensors ingested as 27 blocks plus 9 resident, the overflow refusal NEVER fired, and 126 of the 333 carried values in FP16's subnormal range totalling 167,167 -- so the loss is confined to values a norm cannot see, and the other 207 tensors are exact. The per-block stride is 30,490,624 bytes, which is the 30,479,008 the twelve roles' shapes predict rounded up to the 16 KiB page. Note that page: it is `GTURBO_PAGE_BYTES`, not the OS's 4 KiB, and an assertion hardcoding 4096 reads as a plausible near-miss (the 11,616-byte gap is one 16 KiB page and not three 4 KiB ones).
    **`shape4` TRUNCATES AND THE PATCH EMBEDDING IS RANK 5.** `patch_embed.proj.weight` is `[1152, 2, 16, 16, 3]`, so recording it through `shape4` drops the channel count and leaves an index whose shape's PRODUCT no longer equals the tensor's element count -- bytes intact, every length check passing, and a later reader computing `rows * cols` simply getting a smaller number than the tensor has. `resident_shape` flattens the trailing dims instead, which is also the shape the tensor is USED at: the conv has kernel == stride, so it is a plain GEMM matrix and no conv kernel exists or is wanted. It records `(1152, 1536)`, verified on the real install.
    **The runtime's half is a NAME-SCOPED gate.** `readable_resident_dtype` takes the tensor's name and accepts tag 2 only under `vision.`, because `norm_view` and `read_bf16_host` are dtype-BLIND and decode by byte width -- so an FP16 tensor either reaches is misread rather than rejected. Granting tag 2 outright is not a narrower bug than the one the refusal prevents, it is the same one; `an_fp16_resident_tensor_is_refused_unless_it_is_the_vision_tower` is the half that could otherwise rot, since nothing else says the exception did not widen.


14. **A 429 IS THE ONE STATUS THAT MEANS "TRY AGAIN", AND THE RETRY LADDER TREATED IT AS PERMANENT.** `read_chunk_retrying` returned on ANY non-206 (`Err(e @ UnexpectedStatus { .. }) => return Err(e)`), under a comment whose reasoning was sound for the case it was written for: "a range the server will not serve at all is not going to start working" is true of 404 and 416. It swept in 429 and 5xx, which are the server saying "not now" -- so a rate limit aborted the whole walk on its FIRST occurrence, and this walk CANNOT RESUME, so a twenty-minute 16 GB stream restarted from zero.
