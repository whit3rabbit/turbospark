# Archived module guide: model-io

This is the detailed module guide moved from `crates/model-io/AGENTS.md`. The active entry point is the short guide at `crates/model-io/AGENTS.md`; this page preserves the deeper directory map, commands, gotchas, and evidence notes for on-demand reading.
...
- Contains `unsafe` code restricted to memory-mapping (`mmap`): `resident_buffer.rs` (the resident weight mapping and a `sysconf` page-size read) and `safetensors.rs` (`SafetensorsFile::open`'s `Mmap::map`).
...
|   |   +-- mod.rs              # Manifest re-export & load_manifest/validate/peek_family
|   |   +-- quant.rs            # EXECUTABLE_GGUF_TYPES & validate_quant
|   |   \-- types.rs            # Manifest, ManifestArch, ManifestQuant, ManifestQuantSlot, etc.
...
|   +-- kv_quant.rs             # --kv-bits eligibility policy & KV row byte costs
...
|   +-- ngram_hash.rs           # qwen4_exp's PLE n-gram hash & decode-time NgramContext
|   +-- ngram_hash_tests.rs     # Unit tests for the n-gram hash and shift register
|   +-- ngram_table.rs          # qwen4_exp's hashed n-gram PLE table layout & reader
|   +-- ngram_table_tests.rs    # Unit tests for the n-gram table layout reader
...
- `encoder_config.rs`: `EncoderConfig`/`EncoderQuantization`, deserialized straight from an encoder model's `config.json` (BGE's `model_type: bert`, Snowflake Arctic Embed's `model_type: xlm-roberta`); unrelated to `ArchConfig`, which describes the decode-side families.
...
- `vision_sidecar.rs`: `SidecarRecord`/`is_sidecar_dir`/`load`/`sidecar_arch` for the vision-tower sidecar install format (vision memory sidecar, part A1): a tower installed once as its own small `<alias>.gturbo-vision/` directory (same `manifest.json`/`model_weights.bin`/`packed_experts/layout.json`/`packed_vision/` shape as a full install, at `numLayers: 0`), attachable to any compatible text-only trunk at runtime. `vision_sidecar.json` carries what the manifest structurally cannot state: the `kind` marker itself, which text family and hidden size the tower pairs with (`pairs_with`), and where its bytes came from (`source`).
- `ngram_hash.rs`: `qwen4_exp`'s PLE n-gram hash (`docs/QWEN4_PHASE0.md` item 4, Phase 3 prerequisite) -- `build_layer_multipliers`/`find_nth_prime_after` as CROSS-CHECKS on the checkpoint's own shipped buffers (never the primary source, per `PleConfig`'s doc), `ple_ngram_rows` for the per-token hash, and `NgramContext` for the decode-time EOS-boundary-aware shift register (recurrent state slot 2 in the Phase 0 doc's "three recurrent states" table). **VERIFIED AGAINST `transformers`' ACTUAL SOURCE, NOT AGAINST THIS PROJECT'S OWN SUMMARY OF IT.** `docs/QWEN4_PHASE0.md` originally carried `ctx = [prev_2, prev_1, cur]` for the hash's multiplier pairing, which fetching and reading `modular_qwen4_exp.py` (`fc5c5bde8`) directly showed to be wrong: the multiplier at shift index `s` pairs with the token `s` positions BACK from current (index 0 is the current token, unshifted), not with a fixed oldest-to-newest position. The original formula omitted the current token from the bigram hash entirely -- a plausible-looking formula that would have hashed every n-gram to the wrong row, fluently. `NgramContext`'s update rule is DERIVED from `_shift_right_ignore_eos`'s masking condition (worked through algebraically in the doc) rather than copied, since the reference has no decode-time loop of its own: the token just fed always becomes the new one-back slot regardless of whether it was EOS (the masked value already agrees either way), and every other slot either shifts forward normally or resets outright to `eos_token_id` when the fed token IS `eos_token_id` -- the one case where a plain shift-and-drop register would leak a stale pre-EOS token into the trigram hash. Mutation-checked: reversing the ctx construction order in `NgramContext::step` (reproducing the original bug's shape) reddens exactly the three tests that assert on ctx order and none of the frozen-constant or discrimination ones.
- `ngram_table.rs`: Decodes `qwen4_exp`'s hashed n-gram PLE table (`NgramTableLayout`), 30.8% of that checkpoint at 320,001,536 rows of 160 values. **Does not reuse `PackedExpertsLayout`**, which the vision tower does -- this table has no layer axis and no blob boundary, so borrowing that schema would put two fictional axes on it. **Read by `mmap` rather than the `pread` streamer**, the opposite of the routed experts: a 100-byte row is far under a page, and sub-page random access is the streamer's worst case and the page cache's ordinary one.
...
- `safetensors.rs`: `SafetensorsFile`, a memory-mapped reader for loading tensors directly out of a local `.safetensors` file without copying.
- `kv_quant.rs`: The `--kv-bits` value type (`KvQuant`) plus layer-eligibility and per-row-byte-cost helpers for TurboQuant KV-cache quantization; portable like `context_policy`, no `gpu` dependency. Layer policy is mlx-vlm's own `should_quantize_kv_layer`: quantize every full-attention layer except the last (sliding-window and linear layers are never quantized). The codec's actual numerics live in `turbospark_compute::kv_quant`; see `docs/TRUBOQUANT.md`.
