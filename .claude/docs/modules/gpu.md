# Archived module guide: gpu

This is the detailed module guide moved from `crates/gpu/AGENTS.md`. The active entry point is the short guide at `crates/gpu/AGENTS.md`; this page preserves the deeper directory map, commands, gotchas, and evidence notes for on-demand reading.
...
|   +-- kv_cache_rewind.rs          # KvCacheManager's rewind & reset methods (split out of kv_cache.rs)
|   +-- kv_quant_tables.rs          # TurboQuant per-layer GPU tables: sign vector, Lloyd-Max codebook, midpoints
|   +-- kv_quantize.rs              # Host dispatch for kv_quantize_tq.metal (TurboQuant K/V row quantization)
|   +-- attention_decode.rs         # Split-KV decode attention dispatch
|   +-- attention_decode_tests.rs   # Unit tests for attention decode dispatch
|   +-- attention_indexed.rs        # Decode attention over an explicit position list (port-local, qwen4_exp QSA; dispatched since 2026-09-05)
|   +-- attention_tq.rs             # Decode attention over TurboQuant-quantized K/V rows (port-local)
...
|   +-- ple.rs                      # qwen4_exp's PLE gate (port-local, docs/QWEN4_PHASE0.md section 4)
|   +-- prefill_scratch.rs          # Chunked prefill scratch buffer layout (undispatched)
|   +-- qsa_indexer.rs              # qwen4_exp's QSA block indexer: pooling + scoring + composed advance (port-local; dispatched since 2026-09-05)
|   +-- qsa_indexer_state.rs        # qwen4_exp's QSA indexer state: raw key history + incremental pooled-block cache (port-local; dispatched since 2026-09-05)
...
|       +-- attention_indexed.metal # attention_decode_indexed_partial over a position list (port-local)
|       +-- attention_tq.metal      # Decode attention over TurboQuant-packed K/V rows (port-local)
...
|       +-- ple.metal               # qwen4_exp's PLE gate shader source (port-local)
|       +-- qsa_indexer.metal       # qwen4_exp's QSA block pooling + scoring shader source (port-local)
...
\-- tests/                          # Metal numerical parity & allocation unit tests
    ...
    +-- dispatch_profile.rs
    +-- dsv4_state.rs
    +-- gdn_conv_dilated_parity.rs
    +-- gdn_gated_norm_sigmoid_parity.rs
    +-- gdn_parity.rs
    ...
    +-- power_state.rs
    +-- prefill_scratch.rs
    +-- qsa_indexer_parity.rs
    +-- qsa_indexer_state.rs
    +-- resident_metal.rs
...
- `kv_cache.rs` (+ `kv_cache_rewind.rs` for its rewind/reset methods, split out per this file's own 400-line guidance): `KvCacheManager` managing persistent per-layer Metal KV buffers (linear or sliding-window ring).
- `kv_quant_tables.rs` + `kv_quantize.rs` + `shaders/kv_quantize_tq.metal`, and `attention_tq.rs` + `shaders/attention_tq.metal`: TurboQuant KV-cache quantization's GPU half (`docs/TRUBOQUANT.md`'s `--kv-bits` feature). `kv_quant_tables.rs` builds the sign/codebook/midpoint tables once at open from `turbospark_compute::kv_quant`; `kv_quantize.rs` dispatches the commit-time write (one call per K/V side); `attention_tq.rs` mirrors `attention_decode.rs`'s two-pass split-KV structure (and `attention_indexed.rs`'s sparse variant) over the packed rows, adding the codebook tables every dispatch needs. Read `docs/TRUBOQUANT.md` before touching KV quantization or quoting the assessment it reversed.
...
- `attention_indexed.rs` + `shaders/attention_indexed.metal`: `attention_decode_indexed_partial`, single-token decode attention over an EXPLICIT LIST of key/value positions -- the attention APPLICATION half of `qwen4_exp`'s QSA (`docs/QWEN4_PHASE0.md` section 5: the indexer's block mask "ANDed onto the causal mask"), matched to `turbospark_compute::indexed_attention`. Port-local; no Swift counterpart. **DISPATCHED from `families/qwen4/attn.rs` since 2026-09-05**, above `index_top_k` complete blocks only; at or below that the dense kernel runs unchanged, which is what keeps this family's frozen digests where they are. **IT IS `attention_decode_partial` WITH ONE CHANGE**: the loop walks list indices `[c*chunk_len, min((c+1)*chunk_len, n_sel))` and reads K/V row `positions[i]`, instead of walking positions `[p_start, p_end)`. Everything else -- the block reduction, the online-softmax recurrence, the FP32 accumulators, the `(m, d, o)` partial layout -- is copied verbatim, and pass 2 is attention.metal's OWN `attention_decode_combine`, specialized through `attention_function_constants(scale, 0, num_chunks, false)` exactly as the dense host specializes it (same `constants_key`, Gotcha 1's silent-wrong-pipeline trap avoided by construction). Chunking is `chunks_for(n_sel)` over the LIST length. Runtime `scale`/`num_chunks` buffer arguments rather than the dense kernel's function constants, which is not a numerics difference (the dense FC values equal its runtime ones). Linear KV layout only: the family this serves has no sliding-window layers, so there is no ring arithmetic and the host asserts none. **The design rejected two alternatives, recorded in the shader header**: gathering the selected rows into a compact buffer and running the dense kernel (copies ~2 MiB of K/V per QSA layer per token on the real shape and still needs a gather kernel), and a from-scratch fused sparse kernel (the mlx-vlm on this machine has none -- it builds a boolean mask and calls dense SDPA -- and new online-softmax code is the correctness risk this port's history warns against). `tests/attention_indexed_parity.rs`: a non-contiguous subset, a 2-chunk subset, the real shape (`head_dim` 256, 24/2 heads, 2,051 selected of 3,000, 16 chunks; worst |diff| 2.4e-4 against a 4e-3 tolerance), **the identity list BIT-IDENTICAL to `encode_attention_decode` on the FP32 partial scratch AND the FP16 output** (the equality that lets the dense kernel's real-model verification stand in for this kernel's), and **every unselected K/V row set to NaN with the output required unchanged** (Gotcha 59's trap turned into a stray-read detector). Three mutations checked, each asserted to apply and each reddening exactly its own cases: reading row `i` instead of `positions[i]` (every CPU-compared case; the identity case is blind to it by construction), dropping the chunk offset (every multi-chunk case; the 14-row single-chunk case is blind by construction), and the host always dispatching one chunk (ONLY the bitwise case, and only because it compares the FP32 partials -- **the FP16 output alone let that mutation survive**, since half rounding absorbs reassociation-level differences). **THE FIXTURE HAD TO BE REWRITTEN TWICE before those mutations reddened anything**: keys at magnitude 0.3 made the softmax near-uniform, so the output was the mean of V over WHATEVER rows were read and two mutations passed within tolerance; and `sin(i * 1.3)` over the flat `[seq, heads, head_dim]` index made each key row a small rotation of the previous one, so scores were near-periodic in position and two large row sets sampled that pattern identically (gap 0.0008 on the real shape). Hashed independent rows at key magnitude 3 give O(0.1) gaps, and `assert_fixture_discriminates` now checks that gap against 10x the tolerance BEFORE any parity line is trusted -- AGENTS.md Gotcha 51's rule (a fixture that cannot see the mutation proves nothing), stated as a test.
- `rms_norm.rs`: RMSNorm dispatches (`rmsnorm_no_scale`, `rmsnorm_bf16w`, `rmsnorm_bf16w_centered`, per-head norm variants). The centered one is PORT-LOCAL (Swift reads no architecture with that convention) and its contract is `turbospark_compute::rms_norm_centered`: `out = x * inv * (1 + w)`, the form whose checkpoint weight is an OFFSET FROM UNITY. **It is a separate KERNEL and not a function constant on its plain sibling**, because `muse_glimmer` uses both conventions in ONE forward pass -- four centered per-layer norms and a plain final one -- so a specialization axis whose byte missed `constants_key` would silently make them the same function (Gotcha 1's trap, and Gotcha 10). The `1 +` is applied on the FP32 accumulator and never baked into the stored weight: these weights sit near ZERO and BF16's resolution near 1.0 is 2^-8, so a bake would destroy ~39% of a 0.01 weight's magnitude. `tests/scaled_norm_and_embed.rs` holds the parity case and, beside it, `the_two_norm_conventions_are_different_functions`, which asserts the fixture DISCRIMINATES before the parity case is believed -- at weights near zero the two forms converge and a tidy fixture would pass against either kernel. **`rmsnorm_bf16w_perhead_centered` is the PER-HEAD sibling of that kernel** (the `qwen3_5` MTP head's `q_norm`/`k_norm`), and it is the sharpest case for the per-tensor rule in the repo: the TRUNK of the same model carries tensors of those exact names, at that exact shape, resolved through the same `encode_full_attention_block`, and they are plain. `crates/runtime`'s `QkNormConvention` is what lets the two call sites disagree. Its parity case has the same discriminating guard, with the fixture at 0.78 rather than near zero because that is where the real weights sit; the whole-vector fixture's near-zero range would not have separated the two. **`rmsnorm_bf16w_grouped_centered` (Phase 2 of `qwen4_exp`'s bring-up) is a THIRD sibling and the closest-to-invisible one yet**: `qwen4_exp`'s `hc_norm` (x2 per layer, plus the final mixer) and PLE's `norm_key`/`norm_query`/`norm_conv` take FOUR independent statistics, one per 2560-wide slice of a 10240-wide stream, with a SINGLE 10240-wide weight read at each element's own global index rather than shared across groups the way the per-head kernel's weight is shared across heads (`docs/QWEN4_PHASE0.md` item 9's norm taxonomy, row 1). **It binds the identical buffer shapes as `rmsnorm_bf16w_centered`** -- same `[D]` input, same `[D]` weight, same centered convention -- so unlike every norm pair above, no buffer-size mismatch would catch a call site that reached for the wrong one; the two differ only in whether the reduction pools the whole vector or four slices of it, and `qwen4_exp`'s OWN `pre_fc_norm_hidden` is the row-3 PLAIN full-width tensor at that SAME 10240 width, so both kernels are live in one model at one width. `tests/rms_norm_grouped_parity.rs` holds the parity case (each group phase- AND amplitude-shifted so a mis-derived group stride reads as a wrong number rather than a plausible one) and `the_grouped_and_plain_centered_forms_are_different_functions`, on a fixture whose four groups span 1x to 4x amplitude so the pooled statistic sits well away from any one group's own -- a fixture with uniform group magnitudes cannot tell the two kernels apart at all, since a pooled statistic over four identical-scale slices equals each slice's own.
- `rope.rs`: Rotary positional embedding dispatches (`rope_proportional_neox`, `rope_neox_subdim`, and ROADMAP M5's `rope_neox_freqs`). The first three derive each pair's frequency from a SCALAR theta; `rope_neox_freqs` reads a precomputed per-pair table plus a magnitude scale, because YaRN interpolates per dimension along a ramp and no single number expresses that. The table is position-independent, so it is built once at open by `turbospark_compute::yarn_frequencies`. Incidentally the shape a LEARNED frequency table needs, which is what `rope_freqs.weight` is -- refused by name since M4, and nothing here wires it up. **DO NOT REVISIT: widening the four production `encode_*` dispatches' threadgroup from `(1,1,1)` to `(32,1,1)` is a MEASURED DEAD END.** They dispatch at one thread per threadgroup (31 of 32 SIMD lanes idle per group), which reads like free throughput on an elementwise, no-reduction kernel -- `dispatch_threads` (the non-uniform Metal API these calls use) needs no shader change and no grid change to widen, since every thread still gets its own `thread_position_in_grid` regardless of threadgroup shape. Widened, byte-identical (all three rope parity files plus the vision synthetic suite's frozen digest stayed green, `encode_rope_mrope_interleaved` included via `an_injected_run_has_a_frozen_digest`), and measured on the real `qwen38-27b.gturbo` install: three interleaved pairs, `TURBOSPARK_PHASES=1`, one warmup discarded, AC power, quiet machine -- 21.72 tok/s baseline against 21.74 tok/s widened, a difference an order of magnitude under this machine's normal run-to-run spread. RoPE is evidently too small a share of decode time for its own dispatch shape to move the total; a real measurement of its share (Gotcha 66's `gdn_prefill_share_bench.rs` pattern) would be needed before this is worth re-opening, not another end-to-end tok/s reading.
...
- **`encode_gdn_conv_decode`/`_prefill`/`_tail_update` gained a `dilation: u32` parameter (Phase 2 item 6, PLE's dilated conv, `docs/QWEN4_PHASE0.md` section 4: `kernel_size=4`, `dilation=ngram_size=3`).** This is a SHAPE axis, not a convention one (AGENTS.md's own distinction): `dilation=1` is algebraically the pre-existing kernel, not a different function, so the change generalizes the three existing dispatch signatures rather than baking a second pipeline the way the sigmoid gate did. `tail`'s row count generalizes from `taps - 1` to `(taps - 1) * dilation`, decode's tap read becomes `tail[(j * D) * C + tid]`, prefill's virtual-row index becomes `t + j*D - history`, and `tail_update`'s dispatch grid widens to `(taps - 1) * dilation` rows accordingly. **Every pre-existing call site (8 in `gdn_parity.rs`, 3 in `families/qwen/attn.rs` and `batched_layers.rs`) now passes `dilation=1` explicitly**, and `gdn_parity.rs`'s full seven-test suite reproduced with NO edits beyond that one added argument at each site -- the "provably unmoved" evidence Gotcha 12's sigmoid gate established by a different mechanism (a baked pipeline byte) is established here by the same shape-preserving-at-1 argument the shader's own doc comment states.
  ...
  **A shape trap the test itself had to work around**: `GdnShape::qkv_dim()` (`2*num_k_heads*key_head_dim + num_v_heads*value_head_dim`) is what the conv kernels actually dispatch `channels` from, never a value the fixture picks directly, and `validate()` floors `key_head_dim` at a multiple of 32 -- so a fixture's `CHANNELS` constant has to equal that formula's output (68, at the smallest legal shape) rather than an arbitrary small round number. The first draft picked `CHANNELS = 8` and every buffer silently undersized against the kernel's real dispatch width, reading back zeros and garbage that looked like a broken kernel until the shape arithmetic was checked.
  ...
  Verified on the real `qwen38-27b.gturbo` install (production dispatch of these same three kernels, at `dilation=1`, via `families/qwen`'s GDN mask-2 layers): greedy and sampled smoke tests both reproduced coherent output reaching `MaxTokens`, and `qwen38_quality_gate` reproduced its frozen reference-answer perplexity (4.9432) and all three digests (greedy, sampled, and the 8-slot constrained arm) exactly -- zero numerics drift from the parameter's addition. **`dilation != 1` IS dispatched now**: `families/qwen4/ple.rs`'s PLE conv chain calls `encode_gdn_conv_decode` with `dilation = ngram_context_len + 1` (3 on the real checkpoint), a real production caller since `qwen4_exp`'s Phase 3 landed. `GdnStateManager`'s own SIZING (Gotcha 9 below) still has no production `dilation != 1` caller -- PLE manages its conv tail separately rather than through that manager.
...
- `dequant_q3_k_gemv.rs`: the GGUF Q3_K GEMV dispatch plus an embedding lookup (Dense Qwen2 roadmap item, 2026-09-19), port-local. Resident GEMV and embed lookup, NO routed pair -- the Q6_K footing, since every real Q3_K file in this port's orbit is dense. Q3_K is the K-quant whose every sibling habit is wrong, and each wrong version is byte-aligned and finite: the f16 super-scale is the LAST field of the 110-byte superblock (only Q2_K among the K-quants agrees); the high-bit sign run is indexed by `e % 32` with the BIT chosen by `e / 32`; the sixteen 6-bit scales are shuffled out of twelve bytes through ggml's four-word kmask1/kmask2 aux trick; and a cleared high bit SUBTRACTS 4 from the stored 2-bit level, so reading levels unsigned (the Q6_K habit) returns correctly ordered, non-negative weights. 32 lanes over a 128-element half's 32 quant bytes, so a lane owns one byte per half and eight elements per superblock. Its CPU reference is anchored to ggml itself by a generated oracle (`scripts/ggml_q3_k_oracle.c`, compared with `==` in `crates/compute`'s `q3_k_decodes_exactly_what_ggml_decodes`), so the kernel is two hops from the format's own implementation and each hop has a named test.
...
4. **Two shader sources are CONCATENATIONS, and both for the same reason.** `gdn.rs` passes `dequant_int4.metal` + `gdn.metal` (the fused input projection calls `dequant_int4_gemv_simd_body`, a `static inline` in the former); `moe_gguf/` dispatches pass `moe.metal` + `dequant_q4_k.metal` + `dequant_q6_k.metal` + `dequant_iq_lowbit_tables.metal` + `dequant_iq.metal` + `moe_gguf.metal` (the last uses the first's `RoutedBlobs`, `ExpertOffsets` and `moe_hidden_activation`, plus helpers from the dequantizers). Keep the generated table file before `dequant_iq.metal`: the latter's low-bit kernels reference its five codebooks, so omitting it makes the whole composite library fail MSL compilation even when a request selects an older quant type. Both resolve because the Swift build concatenates every module into one library, which is how these files were always compiled. `concat!` of two `include_str!`s is one `&'static str` with one stable address, which is what Gotcha 1's address-keyed cache needs. Never pass a component file's own constant to a dispatch that wants the concatenation (it would miss the cache, not misbehave), and expect the shared file's kernels to be compiled twice in the process.
...
12. **`gdn_gated_norm` USED TO HARDCODE SILU, WHICH WAS A PER-FAMILY PROPERTY
    ...
    own decode flow (Phase 3) now CALLS the sigmoid encoder --
    `families/qwen4/attn.rs`'s `encode_linear_block` (its mask-2 GDN decode
    path), a real production call site since 2026-09-05.
