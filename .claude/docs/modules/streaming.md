# Archived module guide: streaming

This is the detailed module guide moved from `crates/streaming/AGENTS.md`. The active entry point is the short guide at `crates/streaming/AGENTS.md`; this page preserves the deeper directory map, commands, gotchas, and evidence notes for on-demand reading.
...
11. **THE COPY THIS CRATE EXISTS TO OPTIMIZE IS NOT ALWAYS NECESSARY, and `MappedExpertLayer` is the arm that skips it.** Gotcha 3 says the `pread` is a page-cache memcpy rather than disk I/O and should be optimized as a bandwidth problem. The stronger reading is that a memcpy from the page cache into a pinned slot, so a Metal buffer can wrap the slot, is a copy nothing downstream ever asked for: `crates/gpu`'s `moe_decode.rs` reads expert weights in place from "streamer slots or any other page of memory", so the file's own mapping is an acceptable source and a routed blob pointer can be `mapped_layer_buffer + expert_offset`.

   Measured on the real Gemma 4 install (`docs/EXPERT_RESIDENCY.md`, re-measured 2026-08-29 on AC at the `auto`-resolved 32 slots): peak `phys_footprint` 3,652 MiB streamed against **559 MiB** mapped, decode 53.8 against 68.7 tok/s (1.28x), and output byte-identical on the greedy AND sampled smokes -- against a PRE-CHANGE binary as well as across the seam, since an on-vs-off comparison inside one binary cannot say the default path did not move. The slot cache is 70-90% of the measured peak of every MoE install and it holds a copy of bytes the GPU can read where they already are. Read the throughput row with the caveat on that page and not without it: the capture was not on an idle machine, and contention costs the `pread` arm more than the mapped one, so 1.28x is a ceiling on the honest figure rather than a floor.

   **BOTH TYPES STAY, AND THE TRADE IS THE REASON.** A pinned slot GUARANTEES a bounded working set; a mapping hands residency to the OS, so pages nobody is charged for are pages that can be evicted. Mapped residency is right on a machine that can hold the table and wrong on one that cannot, which is exactly the cold-or-memory-tight case Gotcha 3 already warns about. It is a MODE (`TURBOSPARK_EXPERT_RESIDENCY=mapped`, off by default) and must resolve DOWN to the streamer rather than up.
   ...
   `MappedExpertLayer::expert_offset` is fallible (`Result<u64, StreamerError>`). It must validate the expert index, check arithmetic overflow on `shift + offset + stride`, and assert that the whole blob fits within the mapped buffer before passing offsets to Metal or vision block encoders.
