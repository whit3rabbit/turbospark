...
public final class CronScheduler: @unchecked Sendable {
    ...
    /// Ids taken as due and not yet completed.
    ///
    /// `fireDueJobs` AWAITS its delivery, and nothing about a job changes
    /// while it is suspended there -- `nextFireAt` is rewritten in
    /// `completeFire`, after the await returns. So a second `fireDueJobs`
    /// entered in that window used to take the same job again and deliver
    /// the prompt TWICE. That window is not hypothetical: every `AppModel`
    /// puts a 20-second repeating poll timer on the main run loop
    /// (`startCronScheduler`), so a delivery that outlives one tick
    /// double-submits into the user's chat, and in the test suite -- where
    /// those timers accumulate and are never invalidated -- it is what made
    /// `testCronOneShotFiresAndRemovesItself` intermittently see
    /// `["wake word", "wake word"]` from a single one-shot.
    ...
    /// The file this scheduler owns. INJECTABLE, and not for tidiness.
    ///
    /// A test driving `shared` drives the object the app's poll timers
    /// poll. Every `AppModel` puts a 20-second repeating `fireDueJobs`
    /// timer on the main run loop (`startCronScheduler`) and nothing ever
    /// invalidates it, so in a suite where twenty test files build one a
    /// tick can land inside any `await` -- which is how a case asserting a
    /// single delivery of its own one-shot intermittently saw two. Driving
    /// `shared` also shares one persisted file with every other case. A
    /// test passes its own directory and is reached by neither.
    /// `InputHistoryStore` takes the same seam
